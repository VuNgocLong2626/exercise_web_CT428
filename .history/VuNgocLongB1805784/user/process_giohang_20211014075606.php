<?php
    ...
    if(isset($_POST['tang'])){
        ...
        $result = $conn->query("SELECT * FROM hanghoa hh JOIN hinhminhhoa hmh ON hh.MSHH = hmh.MSHH JOIN loaihanghoa lh ON lh.MaLoaiHang = hh.MaLoaiHang WHERE hh.MSHH='".$_GET['mshhXoa']."'");
        $row = $result->fetch_assoc();
        if(($item_giohang["soluong"] + 1 )<= $row["SoLuongHang"]){
			$soluong=$item_giohang["soluong"]+1;
		}else{
