...
<div class="from-loai">
    <form name="quanlyloai" method="post" action="process/process_quanlyloai.php">
        <div class="mb-3">
            ...
            <input type="text" class="form-control" name="maloai" value = "<?php echo $row_sua['MaLoaiHang']?>">
        </div>
        <div class="mb-3">
            <label for="" class="form-label">Tên Loại</label>
            <input type="text" class="form-control" name="tenloai" value = "<?php echo $row_sua['TenLoaiHang']?>">
        </div>
...
<?php
    ...
    while($row = $result->fetch_assoc()){
        ...
        echo '<tr>
                ...
                <th scope="col">
                <a class="btn btn-secondary btn-size-s" href=process/process_quanlyloai.php?maloaihang=' .$row["TenLoaiHang"]. '">Xóa</a>
