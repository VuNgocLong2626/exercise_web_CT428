<div class="from-sach">
    <form name="quanlysach" method="post" action="process/process_quanlysach.php" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <input class="form-control" type="file" id="hinhanhk" name="hinhanh" >
        </div>
        <div class="mb-3">
        ...
        <button type="submit" class="btn btn-secondary" name="capnhatanh" id="capnhatanh" onmouseover="">Cập nhật ảnh</button>
    </form>
</div>
...
<?php
    $result = $conn->query("SELECT h.MSHH, h.TenHH, hh.TenHinh, lh.TenLoaiHang, h.QuyCach, h.Gia, h.SoluongHang FROM `hanghoa` h JOIN `hinhminhhoa` hh ON hh.MSHH = h.MSHH JOIN `loaihanghoa` lh ON h.MaLoaiHang = lh.MaLoaiHang");
    $i=1;
    while($row = $result->fetch_assoc()){
        echo '<tr>
                ...
                <button onclick="getAllSach(\'' .$row["MSHH"]. '\',\'' .$row["TenHH"]. '\',\'' .$row["QuyCach"].'\',\''.$row["Gia"].'\',\''.$row["SoluongHang"]. '\')" class="btn btn-secondary btn-size-s">Sửa</button>
                </th>
            <tr>';
            $i++;
    }
