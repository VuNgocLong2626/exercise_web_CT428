...
<?php
    $result = $conn->query("SELECT h.MSHH, h.MaLoaiHang, h.TenHH, hh.TenHinh, lh.TenLoaiHang, h.QuyCach, h.Gia, h.SoluongHang FROM `hanghoa` h JOIN `hinhhanghoa` hh ON hh.MSHH = h.MSHH JOIN `loaihanghoa` lh ON h.MaLoaiHang = lh.MaLoaiHang");
    $i=1;
    while($row = $result->fetch_assoc()){
        echo '<tr>
