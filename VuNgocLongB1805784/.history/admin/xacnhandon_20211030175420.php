...
<?php
    $result = $conn->query("SELECT * FROM dathang;");
    while($row = $result->fetch_assoc()){
        ...
        <form name="quanlyloai" method="post" action="process/process_xacnhandon.php?sohd='.$row["SoDonDH"].'">
            <tr>
                ...
                <th scope="col" style="width: 400px;">'.$row["DiaChiGiao"].'</th>
                <th scope="col" style="width: 12%;">'.$row["TrangThaiHD"].'</th>
                <th scope="col" style="width: 200px;">
                    ...
                    <a class="btn btn-secondary" style="margin-top: 5px;" href="index.php?action=chitiethoadon&masohd='.$row["SoDonDH"].'">Chi tiết</a>
                </th>
            </tr>
        </form>
        ';
    ...
    $result = $conn->query("SELECT * FROM dathang WHERE TrangThaiHD != 'Chưa xử lý';");
    while($row = $result->fetch_assoc()){
        echo '
        <form name="quanlyloai" method="post" action="process/process_xacnhandon.php?sohd='.$row["SoDonDH"].'">
            <tr>
                ...
                <th scope="col">'.$row["TrangThaiHD"].'</th>
                <th scope="col">
                    <button class="btn btn-secondary" style="display: block;" name="dangxl">Đang xử lý</button>
                    <button class="btn btn-secondary" style="display: block;margin-top: 5px;"name="daht">Đã Hoàn Thành</button>
                    <a class="btn btn-secondary" style="margin-top: 5px;" href="index.php?action=chitiethoadon&masohd='.$row["SoDonDH"].'">Chi tiết</a>
